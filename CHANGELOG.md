...
- Lamassu: ⚠️ remove `dott_boblingen`, `dott_karlsruhe`, `dott_mannheim`, `dott_darmstadt`, `dott_friedrichshafen`, `dott_lindau`, `dott_ludwigsburg`, `dott_reutlingen`, `dott_tubingen`, `dott_uberlingen`, `dott_ulm`, `dott_saarbrucken`, `dott_bregenz`,  `citiz` feeds

## 2026-09-15

- Lamassu:
  - add `citiz` feed
  - ⚠️ remove `mulhouse` feed
- OCPDB: add datex2_e_clearing_net
...
- Lamassu: add `deer` feed. Deer is now integrated directly from the [GoUrban](https://go.api.gourban.services) GBFS API using the `Authorization-GBFS` header (previously generated by x2gbfs from `deer.fleetster.de`)
- GeoServer: [bump to 2.28.5](https://geoserver.org/announcements/vulnerability/2026/08/14/geoserver-2-28-5-released.html)
