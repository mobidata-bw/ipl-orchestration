...
| OCPDB: initialization script                                                        | `ocpdb-init`         |                      | *-*       | *n/a*              | runs once on startup |
| OCPDB: database ([PostgreSQL](https://www.postgresql.org))                          | `ocpdb-db`           | ✔︎                    | *-*       | *n/a*              |
| OCPDB: Regionalschlüssel importer                                                  | `ocpdb-regionalschluessel-importer` |       | *-*       | *n/a*              | runs once on startup; imports administrative region boundaries (VG25 GeoPackage) into the OCPDB database for [Regionalschlüssel](https://de.wikipedia.org/wiki/Amtlicher_Gemeindeschl%C3%BCssel#Regionalschl%C3%BCssel) assignment |
...
| ParkAPI: database initialization script                                             | `park-api-init-db`   |                      | *-*       | *n/a*              | runs once on startup |
| ParkAPI: converter initialization script                                            | `park-api-init-converter`                   | *-*       | *n/a*              | runs once on startup |
| ParkAPI: database ([PostgreSQL](https://www.postgresql.org))                        | `park-api-db`        | ✔︎                    | *-*       | *n/a*              |
| ParkAPI: heartbeat                                                                  | `park-api-heartbeat` | ✔︎                    | *-*       | *-*                | |
| ParkAPI: Regionalschlüssel importer                                                | `park-api-regionalschluessel-importer` |    | *-*       | *n/a*              | runs once on startup; imports administrative region boundaries (VG25 GeoPackage) into the ParkAPI database for Regionalschlüssel assignment |
| GBFS: [Lamassu](https://github.com/entur/lamassu)                                   | `lamassu`            | ✔︎                    | 8500      | `/sharing/gbfs`http://10.70.172.11:3000/assets    | aggregates (public) [GBFS](https://gbfs.org) vehicle sharing data sources |
| GBFS: Lamassu (admin UI)                                                            | `lamassu`            | ✔︎                    | 9002      | *-*                | allows inspecting Lamassu's operating state |
| GBFS: Lamassu database ([Redis](https://redis.io))                                  | `redis`              | ✔︎                    | *-*       | *n/a*              |
| GBFS: [x2gbfs](https://github.com/mobidata-bw/x2gbfs)                               | `x2gbfs`             | ✔︎                    | *-*       | *-*                | generates [GBFS](https://gbfs.org) feeds from proprietary ride-sharing APIs |
| GBFS: database ([PostgreSQL](https://www.postgresql.org))                           | `ipl-db`             | ✔︎                    | 5432      | *n/a*              | stores fetched GBFS data, served by `geoserver` |
| [Dagster](https://docs.dagster.io/getting-started): task runner                     | `dagster-pipeline`   | ✔︎                    | *-*       | *-*                | runs (custom Python) tasks (e.g. GTFS import, webcam images download) & collects their logs |
...
| Dagster: database ([PostgreSQL](https://www.postgresql.org))                        | `dagster-postgresql` | ✔︎                    | *-*       | *n/a*              |
...
| RadVIS: database ([PostgreSQL](https://www.postgresql.org))                         | `ipl-db`             | ✔︎                    | 5432      | *n/a*              | stores RadVIS data, served by `geoserver` |
| GTFS: API ([PostgREST](https://postgrest.org/))                                     | `gtfs-api`           | ✔︎                    | 4000      | `/gtfs`            | serves [GTFS](https://gtfs.org/schedule/) data in `gtfs-db` as an API |
| GTFS: documentation ([Stoplight Elements](https://github.com/stoplightio/elements)) | `gtfs-api-docs`      | ✔︎                    | 4001      | `/docs/gtfs`       | UI documenting the GTFS API |
...
| GTFS: database ([PostgreSQL](https://www.postgresql.org))                           | `gtfs-db`            | ✔︎                    | *-*       | *n/a*              | stores imported GTFS data, served by `gtfs-api` and `geoserver` |
| database proxy ([pgbouncer](https://www.pgbouncer.org))                             | `pgbouncer`          | ✔︎                    | 6432      | *n/a*              | proxies services' connections to databases (`dagster-postgresql`, `gtfs-db`, etc.) |
| task/message queue ([RabbitMQ](https://www.rabbitmq.com))                           | `rabbitmq`           | ✔︎                    | *-*       | *-*                |
| web server ([Caddy](https://caddyserver.com)): static data                          | `caddy`              | ✔︎                    | 6999      | `/datasets`        | serves static datasets |
| web server ([Caddy](https://caddyserver.com)): webcam images                        | `caddy`              | ✔︎                    | 6999      | `/webcams`         | webcam images pulled by `dagster-pipeline` |
| web server ([Caddy](https://caddyserver.com)): OCPDB images                         | `caddy`              | ✔︎                    | 6999      | `/ocpdb/images`    | serves OCPDB images |
| web server ([Caddy](https://caddyserver.com)): private GBFS feeds                   | `caddy`              | ✔︎                    | 6999      | `/gbfs`            | serves private GBFS feeds generated by `x2gbfs` |
| web server ([Caddy](https://caddyserver.com)): meta files                           | `caddy`              | ✔︎                    | 6999      | `/.well-known`, `/robots.txt`, `/favicon.ico` | serves miscellaneous files (not datasets) |
| web server ([Caddy](https://caddyserver.com)): requests statistics                  | `caddy`              | ✔︎                    | 6998      | *-*                | serves reports generated by `goaccess` |
